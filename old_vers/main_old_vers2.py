    ...
    if attempt < min(2, len(top_indices)) and probabilities[top_indices[attempt]] >= 1e-4:
        ...
        if predicted_class == 'schedule':
            ...
            if len(surnames) == 0 and len(group_num) >= 1:
                ...
                return
            #elif len(surnames) >= 1:
                #await state.update_data(surname=surnames[0])
                #await state.update_data(predicted_class=predicted_class)
                ##schedule_links = get_teacher_schedule_links(surnames[0])
                #if not(schedule_links is None):
                    # Добавить клавиатуру с ответом
                    #(f"Ссылка на расписание занятий: {schedule_links.get('class_schedule')}")
                    #(f"Ссылка на расписание сессии: {schedule_links.get('exam_schedule')}")
                #else:
                    #ничего не делать просто отправить сообщение как обычно в соответствии с классом
                return
